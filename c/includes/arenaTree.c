...
#line 48 "/home/phil/c/z/arenaList/arenaList.c"
...
#line 54 "/home/phil/c/z/arenaList/arenaList.c"
...
#line 25 "/home/phil/c/z/arenaTree/arenaTree.c" // CC
...
#define makeLocalCopyOfArenaTreeKey(string,stringLength,node) const typeof(content__ArenaTreeNode(&node)->length) stringLength = content__ArenaTreeNode(&node)->length; char string[stringLength+1]; string[stringLength] = 0; memcpy(string, key_pointer__ArenaTreeNode(&node), stringLength); // Copy the key and the length of the key of the specified node to the stack.

//D1 Pointers, offsets and allocations                                          // Locate items allocated in the arena

#line 113 "/home/phil/c/z/arenaList/arenaList.c"
static ArenaTree makeArenaTreeWithWidth                                                         //I Create a new ArenaTree with the specified width for the optional user data associated with each node.
 ...
 {ArenaTreeArena * a = alloc(sizeof(ArenaTreeArena));                                           // Allocate arena description
  memset(a, 0, sizeof(ArenaTreeArena));                                                 // Clear arena description
  const typeof(newArenaTree(({struct ArenaTree t = {arena: a, proto: &ProtoTypes_ArenaTree}; t;}))) l = newArenaTree(({struct ArenaTree t = {arena: a, proto: &ProtoTypes_ArenaTree}; t;}));                                                          // ArenaTree we are creating
...
#line 129 "/home/phil/c/z/arenaList/arenaList.c"
...
#line 213 "/home/phil/c/z/arenaList/arenaList.c"
...
#line 218 "/home/phil/c/z/arenaList/arenaList.c"
static size_t width_size__ArenaTree                                                     //I Get the width of the data area in a node for a ArenaTree
 (const ArenaTree * list)                                                               // ArenaTree
 {return list->arena->width;
 }
#line 223 "/home/phil/c/z/arenaList/arenaList.c"
static size_t width_size__ArenaTreeNode                                                 //I Get the width of the data area for a node
 (const ArenaTreeNode * node)                                                           // ArenaTreeNode
 {return node->list.arena->width;
 }
#line 228 "/home/phil/c/z/arenaList/arenaList.c"
...
#line 234 "/home/phil/c/z/arenaList/arenaList.c"
...
#line 239 "/home/phil/c/z/arenaList/arenaList.c"
...
#line 254 "/home/phil/c/z/arenaList/arenaList.c"
...
#line 277 "/home/phil/c/z/arenaList/arenaList.c"
...
#line 329 "/home/phil/c/z/arenaList/arenaList.c"
...
#line 367 "/home/phil/c/z/arenaList/arenaList.c"
...
#line 372 "/home/phil/c/z/arenaList/arenaList.c"
static ArenaTreeNode node_ArenaTreeNode__ArenaTree_string_size                                          //I Create a new ArenaTree node with the specified key.
 ...
 {const typeof(sizeof(ArenaTreeNode) + length + list->proto->width(list)) s = sizeof(ArenaTreeNode) + length + list->proto->width(list);                                    // Minimum width of node
#ifndef ArenaTreeEditable
...
#line 612 "/home/phil/c/z/arenaList/arenaList.c"
...
#line 46 "/home/phil/c/z/arenaTree/arenaTree.c" // CC

static void free_ArenaTree                                                              // Free a ArenaTree in its entirety
 (ArenaTree * tree)                                                                     // ArenaTree
...
static  ArenaTreeFound find_ArenaTreeFound_ArenaTreeNode_string                                         //P Find a key if it exists within the ArenaTree owned by the specified node.
 ...
 {const typeof(*Node) node = *Node;
  typeof(makeArenaTreeFound(node.list, key, length)) f = makeArenaTreeFound(node.list, key, length);                                       // Status of find
...
static  ArenaTreeNode   add_ArenaTreeNode_ArenaTree_string                                              // Add a new key if not already present in the specified base ArenaTree.
 (ArenaTree     *       tree,                                                           // The ArenaTree to search
  ...
  const size_t  length)                                                         // Length of the key to find
 {typeof(tree->proto->find(tree, key, length)) f = tree->proto->find(tree, key, length);                                                 // Try to find the key
...
static  ArenaTreeNode   add_ArenaTreeNode_ArenaTreeNode_string                                          // Add a new key if not already present in the ArenaTree owned by the specified node.
 (const ArenaTreeNode * node,                                                           // The ArenaTree to search
  char  * const key,                                                            // The key to add
  const size_t  length)                                                         // Length of the key to find
 {const typeof(node->list) tree = node->list;                                                           // ArenaTree to search
      typeof(node->proto->find(node, key, length)) f = node->proto->find(node, key, length);                                             // Try to find the key
  if (!f.last.proto->valid(&f.last))                                                          // Empty ArenaTree
   {const typeof(tree.proto->node(&tree, key, length)) root = tree.proto->node(&tree, key, length);                                            // Add new key in a node
...
static StringBuffer print_ArenaTreeNode                                                 // Print the tree below the specified node as a string.
 ...
 {typeof(0ul) height = 0ul;                                                                 // Height of root
  typeof(makeStringBuffer()) p = makeStringBuffer();                                                       // Print to this string buffer
...
void test0()                                                                    //TnewArenaTree //Tadd //Tleft //Tright //Tkey //Tup //Tfree
 {   typeof(makeArenaTree()) t = makeArenaTree();
     t.proto->add(&t, "b",  1);
     t.proto->add(&t, "a",  1);
     t.proto->add(&t, "c",  1);
     t.proto->add(&t, "bb", 2);
     ...
     const typeof(t.proto->root(&t)) b = t.proto->root(&t); const typeof(b.proto->left(&b)) a = b.proto->left(&b); const typeof(b.proto->right(&b)) c = b.proto->right(&b); const typeof(c.proto->left(&c)) B = c.proto->left(&c); const typeof(c.proto->right(&c)) C = c.proto->right(&c);

  assert(  b.proto->keyEquals(&b, "b",  1));
  assert(  a.proto->keyEquals(&a, "a",  1));
  assert(  c.proto->keyEquals(&c, "c",  1));
  assert(  B.proto->keyEquals(&B, "bb", 2));
  assert(  C.proto->keyEquals(&C, "cc", 2));
  assert(  b.proto->isRoot(&b));
  assert(  b.proto->equals(&b, a.proto->up(&a)));
  assert(  b.proto->equals(&b, c.proto->up(&c)));
  assert(  c.proto->equals(&c, B.proto->up(&B)));
  assert(  c.proto->equals(&c, C.proto->up(&C)));
...
void test2()                                                                    //Tcount
 {typeof(makeArenaTree()) t = makeArenaTree(); const typeof(100ul) N = 100ul; char c[256];
...
void test3()
 {typeof(makeArenaTree()) t = makeArenaTree(); const typeof(256*256ul) N = 256*256ul; char c[256];
...
void test4()                                                                    //Theight //Tlocate //Troot
 {typeof(makeArenaTree()) t = makeArenaTree(); char c[16]; const typeof(10) N = 10;
...
void test5()
 {typeof(makeArenaTree()) t = makeArenaTree(); char c[16]; const typeof(20) N = 20;
...
void test6()                                                                    //Tcheck //Tsprint //TisRoot //TkeyEquals
 {typeof(makeArenaTree()) t = makeArenaTree(); char c[128]; const typeof(10) N = 10;
...
void test8()                                                                    //Tprint
 {typeof(makeArenaTree()) t = makeArenaTree();  char s[256];  typeof(10) N = 10;
  ...
  assert( H.proto->printsAs(&H, "1   97\n" "0 98..\n" "1   99\n"));
//◉1   97
//◉0 98..
//◉1   99
//◉
