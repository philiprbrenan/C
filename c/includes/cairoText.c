...
typedef struct CairoTextText                                                            // Draw some text
 {const struct ProtoTypes_CairoText *proto;                                             // Methods associated with an arena tree
...
//static CairoText makeCairoText                                                                  // Create a new arena tree
// ()                                                                             // ArenaTree allocator
// {return newCairoTextText(({struct CairoTextText t = {proto: &ProtoTypes_CairoTextText};   t;}));                                                             // Arena tree we are creating
// }
...
void createImage                                                                // Create an image by drawing on a surface
 ...
 {cairo_surface_t * surface = cairo_image_surface_create                        // Cairo
  ...
  const typeof(cairo_ft_font_face_create_for_ft_face(freeTypeFontFace, 0)) cairoFontFace = cairo_ft_font_face_create_for_ft_face(freeTypeFontFace, 0);
