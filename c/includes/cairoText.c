...
static CairoTextCompactList makeCairoTextCompactList                                            // Define a compact list
 ...
 {CairoTextCompactList l = newCairoTextCompactList(({struct CairoTextCompactList t = {proto: &ProtoTypes_CairoTextCompactList};   t;}));
  l.image                = image;
  ({typeof(l.frameColour) sourcesourcesource = makeColour(0,0,0,1); memcpy(&l.frameColour, &sourcesourcesource, sizeof(l.frameColour));});
  ({typeof(l.possibilityColour) sourcesourcesource = makeColour(0,0,0,1); memcpy(&l.possibilityColour, &sourcesourcesource, sizeof(l.possibilityColour));});
  ({typeof(l.pointedColour) sourcesourcesource = makeColour(0,0,1,0.3); memcpy(&l.pointedColour, &sourcesourcesource, sizeof(l.pointedColour));});
  ({typeof(l.prefixColour) sourcesourcesource = makeColour(0,1,0,1); memcpy(&l.prefixColour, &sourcesourcesource, sizeof(l.prefixColour));});
  ({typeof(l.prefixColour) sourcesourcesource = makeColour(0,1,0,1); memcpy(&l.prefixColour, &sourcesourcesource, sizeof(l.prefixColour));});
  ({typeof(l.list) sourcesourcesource = list; memcpy(&l.list, &sourcesourcesource, sizeof(l.list));});
  ({typeof(l.cursorEntry) sourcesourcesource = list.proto->first(&list); memcpy(&l.cursorEntry, &sourcesourcesource, sizeof(l.cursorEntry));});
  l.prefixLength         = 0;
  ({typeof(l.prefixFont) sourcesourcesource = CairoTextFont_sansItalic; memcpy(&l.prefixFont, &sourcesourcesource, sizeof(l.prefixFont));});
  ({typeof(l.possibilityFont) sourcesourcesource = CairoTextFont_serif; memcpy(&l.possibilityFont, &sourcesourcesource, sizeof(l.possibilityFont));});
  l.possibilityFontSize  = 100;
  ({typeof(l.drawTable) sourcesourcesource = drawTable; memcpy(&l.drawTable, &sourcesourcesource, sizeof(l.drawTable));});
  return l;
 }
  ...
  ArenaListfeⁱ(word, cl->list)                                                  // Each word
   {if (word.offset >= cl->startAt.offset)                                      // Words in the scrolled to area
     {makeLocalCopyOfArenaListKey(K, L, word);
      ...
      ({typeof(entryRectangles[wordⁱ]) sourcesourcesource = R; memcpy(&entryRectangles[wordⁱ], &sourcesourcesource, sizeof(entryRectangles[wordⁱ]));});                                               // Rectangle containing this entry up to the next tab stop
...
void test4()                                                                    // Text table using tab stops
 {void draw(CairoTextImage image)
   {const typeof(makeArenaListFromWords("aaaa qqbbbb qqcc qqdd qqee qqff qqgggg qqhh qqiii qqjj qqkkk qql mmmm nn oo qqppppp qq qqrrrr s qqtttttt qquuuu v wwwwww xxx yy zz")) list = makeArenaListFromWords("aaaa qqbbbb qqcc qqdd qqee qqff qqgggg qqhh qqiii qqjj qqkkk qql mmmm nn oo qqppppp qq qqrrrr s qqtttttt qquuuu v wwwwww xxx yy zz");
    ...
    ({typeof(cl.cursorEntry) sourcesourcesource = list.proto->findFirst(&list, "qqee"); memcpy(&cl.cursorEntry, &sourcesourcesource, sizeof(cl.cursorEntry));});                                  // Cursor entry
