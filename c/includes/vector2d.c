...
void test0()                                                                    //TmakeVector2d //Teq //TVector2dnear //TVector2dzero //TVector2done //Tswap //Tclone //Tplus //Tmultiply //Td //Td2 //Tl //Tl2 //Tdot //Tr90 //Tr180 //Tr270 //Tsine //Tcosine //Tarea //Tangle //TsmallestAngleToNormalPlane //Tn //Tzero //Tdivide //Tminus
 {typeof(Vector2dZ()) z = Vector2dZ(); typeof(Vector2dX()) x = Vector2dX(); typeof(Vector2dY()) y = Vector2dY(); typeof(x.proto->r180(&x)) X = x.proto->r180(&x); typeof(y.proto->r180(&y)) Y = y.proto->r180(&y);

  assert( z.proto->zero(&z));
  assert( x.proto->eq(&x, makeVector2d(1, 0)));
  assert( y.proto->eq(&y, makeVector2d(0, 1)));
  assert( z.proto->eq(&z, x.proto->minus(&x, x)));
  ...
  typeof(x.proto->multiply(&x, 3)) x3 = x.proto->multiply(&x, 3); typeof(y.proto->multiply(&y, 4)) y4 = y.proto->multiply(&y, 4); typeof(x3.proto->plus(&x3, y4)) h5 = x3.proto->plus(&x3, y4);
  ...
  assert( Vector2dnear(z.proto->d2(&z, h5) , 25));
    ...
    typeof(h5.proto->divide(&h5, 5)) H5 = h5.proto->divide(&h5, 5);
  ...
  typeof(makeVector2d(1, 1)) xy = makeVector2d(1, 1);  assert( xy.x == 1);  assert( xy.y == 1);
