#line 0 "/home/phil/c/z/byteString/byteString.c"
...
#define copy_bytes_to_local_string(bytes, str) char str[bytes.length + 1]; bytes.proto->copy(bytes, str); // Allocate a string string on the stack and copy the specified bytes into it adding a zero terminator at the end of the local string.
...
void test0()                                                                    //Tcompare //Tcopy
 {BytesAllocator  a = newBytesAllocator(NULL);
  ...
  ByteString  b = a.proto->bytes(a, s, strlen(s));
  assert    (b.proto->compare(b, s) == 0);

  char B[256]; b.proto->copy(b, B);         assert(strcmp(B, s) == 0);
